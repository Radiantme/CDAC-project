...
int main(void)
{
  ...
  float rs_ro_ratio = MQRead() / Ro;
    ...
    uint16_t lpg_val  = MQGetGasPercentage(rs_ro_ratio, GAS_LPG);
      ...
      lcd_clear();  // Clear the LCD
      lcd_set_cursor(0, 0);  // Set the cursor to the first row, first column
      lcd_write_string(lpg_str);  // Write the adc_value to the LCD
    ...
    uint16_t co_val  = MQGetGasPercentage(rs_ro_ratio, GAS_CO);
      ...
      lcd_set_cursor(0, 9);  // Set the cursor to the first row, first column
      lcd_write_string(co_str);  // Write the adc_value to the LCD

    char smoke_str[16];

    uint16_t smoke_val  = MQGetGasPercentage(rs_ro_ratio, GAS_SMOKE);

      sprintf(smoke_str, "SMOKE:%u\r\n",smoke_val);

      HAL_UART_Transmit(&huart1, (uint8_t*)smoke_str, strlen(smoke_str), HAL_MAX_DELAY);
      lcd_set_cursor(1, 0);  // Set the cursor to the first row, first column
      lcd_write_string(smoke_str);  // Write the adc_value to the LCD
